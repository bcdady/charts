...
{{- $secretName := .Values.sasl.existingSecret -}}
{{- if and (include "kafka.saslEnabled" .) (or (empty $secretName) .Values.tls.autoGenerated ) -}}
    {{- true -}}
{{- end -}}
{{- end -}}
...
{{- printf "%s%s" (include "kafka.zookeeper.fullname" .) (tpl .Values.zookeeperChrootPath .) -}}
{{- else -}}
...
zookeeper.ssl.keyStore.location=/config/certs/zookeeper.keystore.jks
zookeeper.ssl.trustStore.location=/config/certs/zookeeper.truststore.jks
...
- name: kafka-init
  ...
  env:
    ...
    {{- end }}
    {{- if and .context.Values.tls.zookeeper.enabled .context.Values.tls.zookeeper.passwordsSecret }}
    - name: KAFKA_ZOOKEEPER_TLS_KEYSTORE_PASSWORD
      valueFrom:
        secretKeyRef:
          name: {{ .context.Values.tls.zookeeper.passwordsSecret }}
